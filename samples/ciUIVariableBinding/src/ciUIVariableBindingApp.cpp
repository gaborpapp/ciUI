...
class ciUIVariableBindingApp : public AppBasic {
public:
...
void ciUIVariableBindingApp::prepareSettings( Settings *settings )
{
...
{    
    ...
    gui->addWidgetDown(new ciUI2DPad(padWidth, padHeight, Vec2f(0,getWindowWidth()), Vec2f(0,getWindowHeight()), &position, "POSITION")); 
    ...
    gui->registerUIEvents(this, &ciUIVariableBindingApp::guiEvent);
}

void ciUIVariableBindingApp::update()
{
    gui->update(); 
}

void ciUIVariableBindingApp::draw()
{
...
void ciUIVariableBindingApp::shutDown()
{
    delete gui; 
}

void ciUIVariableBindingApp::guiEvent(ciUIEvent *event)
{
    //Because of variable binding you don't need to write the code below :)
    //Program values are modified auto-magically...and if you still need the events below they are avaliable for use. 
...
void ciUIVariableBindingApp::keyDown( KeyEvent event )
{
    if(event.getChar() == 'g')
    {
        gui->toggleVisible(); 
...
CINDER_APP_BASIC( ciUIVariableBindingApp, RendererGl )
