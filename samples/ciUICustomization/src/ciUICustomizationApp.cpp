...
class ciUICustomizationApp : public AppBasic 
{    
public:
...
Renderer* ciUICustomizationApp::prepareRenderer()
{
    return new RendererGl( RendererGl::AA_MSAA_2 ); 
}

void ciUICustomizationApp::prepareSettings( Settings *settings )
{
    settings->setWindowSize( 768, 1024 );
    settings->setFrameRate( 60.0f );
}

void ciUICustomizationApp::setup()
{
    gl::enableAlphaBlending(); 
	setFpsSampleInterval(1.0); 
    
    mImage = loadImage( loadResource( "biorhythm.png" ) ); 
    mImageSurface = loadImage ( loadResource( "biorhythm.png" ) );
    buffer = new float[128];
    menuSpacer = CI_UI_GLOBAL_WIDGET_SPACING; 
    menuWidth = (getWindowWidth()-4*menuSpacer) /3.0; 
    menuHeight = getWindowHeight()-menuSpacer*2;
...
void ciUICustomizationApp::update()
{
    for(int i = 0; i < 128; i++)
    {
...
void ciUICustomizationApp::draw()
{
...
void ciUICustomizationApp::shutDown()
{
    delete[] buffer; 
    delete gui; 
    delete gui1; 
...
void ciUICustomizationApp::keyDown( KeyEvent event )
{
...
void ciUICustomizationApp::keyUp( KeyEvent event )
{
}

void ciUICustomizationApp::guiEvent(ciUIEvent *event)
{
...
void ciUICustomizationApp::setupGUI1()
{
    float w = widgetWidth; 
    float h = dim; 
    float spacerHeight = 2; 
    gui = new ciUICanvas(menuSpacer,menuSpacer,menuWidth,menuHeight);
    ...
    gui->registerUIEvents(this, &ciUICustomizationApp::guiEvent); 
}

void ciUICustomizationApp::setupGUI2()
{
    float w = widgetWidth; 
    float h = dim;
    float spacerHeight = 2; 
    gui1 = new ciUICanvas(gui->getRect()->getX()+gui->getRect()->getWidth()+menuSpacer,menuSpacer,menuWidth,menuHeight);
    ...
    gui1->registerUIEvents(this, &ciUICustomizationApp::guiEvent);     
}

void ciUICustomizationApp::setupGUI3()
{
    ...
    float h = dim;
    float spacerHeight = 2;     
    gui2 = new ciUICanvas(gui1->getRect()->getX()+gui1->getRect()->getWidth()+menuSpacer,menuSpacer,menuWidth,menuHeight);
    gui2->addWidgetDown(new ciUILabel("EVEN MORE!!", CI_UI_FONT_LARGE));    
    ...
    gui2->registerUIEvents(this, &ciUICustomizationApp::guiEvent);     
}

CINDER_APP_BASIC( ciUICustomizationApp, RendererGl )
